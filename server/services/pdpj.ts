...
function getConfig(): PdpjConfig {
  return {
    baseUrl: process.env.PDPJ_PORTAL_BASE_URL || "https://portal-servicos.pdpj.jus.br",
    ssoTokenUrl: process.env.PDPJ_SSO_TOKEN_URL || "https://sso.cloud.pje.jus.br/auth/realms/pdpj/protocol/openid-connect/token",
...
function buildIntercorrentePayload(protocolPackage: any) {
...
async function getServiceToken(config: PdpjConfig): Promise<string> {
  if (!config.clientId || !config.clientSecret) {
    throw new Error("PDPJ_CLIENT_ID e PDPJ_CLIENT_SECRET nao configurados.");
  }
  ...
  if (!data.access_token) {
    throw new Error("SSO PDPJ nao retornou access_token.");
  }
...
export const pdpjService = {
  ...
  buildIntercorrentePayload,

  validateIntercorrente(protocolPackage: any) {
    const payload = buildIntercorrentePayload(protocolPackage);
    const errors: string[] = [];
    const warnings: string[] = [];

    if (!payload.numeroProcesso) errors.push("Numero do processo e obrigatorio para peticao intercorrente.");
    if (!protocolPackage.mainDocumentHtml) errors.push("Peca principal nao encontrada no pacote.");
    if (!payload.documentoPrincipal?.titulo) errors.push("Titulo do documento principal e obrigatorio.");
    if (!payload.tribunal) warnings.push("Tribunal nao informado; escolha a tramitacao manualmente.");
    if (!payload.documentos.length) warnings.push("Nenhum anexo registrado alem da peca principal.");
  ...
  async submitIntercorrente(protocolPackage: any) {
    ...
    if (!validation.ok) {
      return {
        ...
        submitted: false,
        validation,
        message: "Pacote com pendencias obrigatorias. Corrija antes de enviar ao PDPJ.",
      };
    }
    ...
    if (!config.enableRealSubmit) {
      return {
        ...
        submitted: false,
        validation,
        message: "Dry-run PDPJ: pacote validado. O envio real ainda esta desativado.",
      };
    }

    if (!config.submitPath) {
      throw new Error("PDPJ_INTERCORRENTE_SUBMIT_PATH nao configurado.");
    }
    ...
    return { dryRun: false, submitted: true, validation, response: responseBody };
  },
};
