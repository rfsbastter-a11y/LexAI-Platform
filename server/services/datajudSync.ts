...
export async function triggerEscavadorOnlySync(tenantId: number = 1) {
  ...
  const progress: { running: boolean; total: number; current: number; synced: number; errors: number; newMovements: number; currentCase: string; completedAt?: number } = {
    running: true,
    total: syncCases.length,
    current: 0,
    synced: 0,
    errors: 0,
    newMovements: 0,
